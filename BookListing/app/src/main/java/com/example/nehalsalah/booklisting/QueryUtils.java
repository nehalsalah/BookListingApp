            ...
            for (int i = 0; i < BookListingArray.length(); i++) {
                ...
                if (authorArray != null) {
                    for (int j = 0; j < authorArray.length(); j++) {
                        //add all authors
                        authors = authors + authorArray.getString(j) + "  ";
                    }
                } else {
                    authors = authors + "Author unknown";
                }
                Log.d("QueryUtils", "Value: " + authors);

                // Extract the value for the key called "description"
                String description = "Description: ";
                if (volumeInfo.has("description")) {
                    description = description + volumeInfo.getString("description");
                } else {
                    description = description + "there is no description here";
                }
                Log.d("QueryUtils", "Value: " + description);
                ...
                BookListing bookListing = new BookListing(title, authors, description, url);
                number++;
